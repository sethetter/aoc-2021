fn main() {
    ...
    let heatmap = build_heatmap(input);
...
fn test_part_1() {
    ...
    assert_eq!(build_heatmap(input), vec![
...
fn build_heatmap(input: String) -> HeatMap {
    ...
    for line in parsed_coords {
        heatmap = add_line_to_heatmap(heatmap, line);
    }
...
fn add_line_to_heatmap(mut heatmap: HeatMap, ((x1, y1), (x2, y2)): Line) -> HeatMap {
    // vertical
    if x1 == x2 {
...
fn print_heatmap(heatmap: HeatMap) {
    for y in 0..heatmap.len() {
        println!("{}", heatmap[y].clone().into_iter().map(|x| x.to_string()).collect::<Vec<String>>().join(""));
    }
}

// fn point_is_on_line(((x1, y1), (x2, y2)): Line, (x, y): Point) -> bool {
